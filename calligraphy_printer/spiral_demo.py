...
class GRBLGCodeStreamer:
    ...
    def _read_line_nonblocking(self) -> str | None:
        ...
        if first_newline > 0:
            line, self.read_buffer = self.read_buffer[:first_newline], self.read_buffer[first_newline+1:]
            if self.verbose:
                LOG.info("<<|%s", line)
            return line
        return None
        ...
        while 1:
            ...
            if grbl_out is not None:
                if 'error' in grbl_out:
                    LOG.error("Received error from Grbl: response [%s]", grbl_out.strip())
                    return False
    ...
    def _safe_to_send(self, num_chars: int) -> int:
        return sum([len(x) for x in self.sent_command_buffer]) + num_chars < self.RX_BUFFER_SIZE
    ...
    def send_setting(self, data: str, timeout: float = 1.0):
        ...
        assert len(self.sent_command_buffer) == 0, "Can't interleave commands and settings."
        self._write(data + "\n")
        ...
        while 1:
            ...
            while self.ser.inWaiting() > 0:
                ...
                if "ok" in rec:
                    ...
                    raise RuntimeError("Received error, corresponding command [%s]", self.sent_command_buffer[0])
...
if __name__ == "__main__":
    ...
    def get_servo(val):
        # val on [0, 1] to a servo speed value
        pwm_period = 1 / (62500 / 1024)
        servo_min = 0.001
        servo_max = 0.002
        return (servo_min + (servo_max - servo_min) * val) / pwm_period * 1000

    s = get_servo(0.0 * np.sin(ts) * 0.5 + 0.5)
    ...
    for t, x, y, s in zip(ts, x, y, s, strict=True):
        ...
        feed_rate = np.clip((move_distance / dt) * 60.0, 1, 6000)
        command = f"G1 X{x:0.2f} Y{y:0.2f} F{int(feed_rate)} S{int(s)}"
