...
class CanvasImager():
    ...
    '''
    def __init__(self, output_width: int, output_height: int, horizontal_padding: int = 100):
    ...
    def update(self, include_debug_drawing: bool = False) -> CanvasImagerOutput:
        ...
        tags = {
            tag.tag_id: tag for tag in tags
        }
        tag_ids = [0, 1, 2, 3]
        ...
        for tag_id, corner_index in zip(tag_ids, corner_indices, strict=True):
            ...
            detection_pts.append(
                tag.corners[corner_index, :]
            )
        if len(detection_pts) == 4:
            detection_pts = np.stack(detection_pts, axis=0)
            result_pts = np.array([[-self.horizontal_padding, 0], [self.output_width-1 + self.horizontal_padding, 0], [self.output_width-1 + self.horizontal_padding, self.output_height-1], [-self.horizontal_padding, self.output_height-1]],)
            # CV2 is very picky that these are float32s.
            M = cv2.getPerspectiveTransform(detection_pts.astype(np.float32), result_pts.astype(np.float32))
            rectified_canvas = cv2.warpPerspective(frame, M, (self.output_width, self.output_height))
        else:
            ...
            for tag in tags.values():
                for idx in range(len(tag.corners) + 1):
                    cv2.line(frame, tuple(tag.corners[idx-1, :].astype(int)), 
                            tuple(tag.corners[idx % len(tag.corners), :].astype(int)), (0, 0, 255), 2)
            
            # Drop crop region box
            if rectified_canvas is not None:
                for idx in range(len(detection_pts) + 1):
                    cv2.line(frame, tuple(detection_pts[idx-1, :].astype(int)), 
                            tuple(detection_pts[idx % len(detection_pts), :].astype(int)), (0, 255, 0), 2)
                    
        return CanvasImagerOutput(
            rectified_canvas=rectified_canvas, 
            debug_image=frame
        )

if __name__ == "__main__":
    imager = CanvasImager(output_width=640, output_height=640)
    show_debug = True
