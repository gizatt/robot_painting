...
def fit_affine_transform(robot_coords : np.ndarray, pixel_coords: np.ndarray) -> np.ndarray:
    """
    ...
    R = np.linalg.lstsq((pixel_coords - pixel_coords_mean).T, (robot_coords - robot_coord_mean).T)[0].T
...
class ImageBlobDetector():
    ...
    def detect_difference_image_peak(self, image: cv2.Mat, last_image: cv2.Mat) -> np.ndarray:
        difference = 1. - ((1. - image) - (1. - last_image))
        keypoints = self.detector.detect((difference * 255.).astype(np.uint8))
        if len(keypoints) != 1:
...
if __name__ == "__main__":
    ROBOT_BOUNDS = [297., 210.] # mm
    X, Y = np.meshgrid(
        np.linspace(0., ROBOT_BOUNDS[0], 5),
        np.linspace(0., ROBOT_BOUNDS[1], 5)
    )
    ...
    IM_SIZE = [640, 480]
    M_gt = np.array([
        [0., 0.8 * ROBOT_BOUNDS[0] / IM_SIZE[1], -30],
        [1.2 * ROBOT_BOUNDS[1] / IM_SIZE[0], 0., -30]
    ])
    pixel_coords = invert_affine_tf(M_gt) @ make_homog(robot_coords) + np.random.normal(0., 5, size=robot_coords.shape)
    ...
    plt.scatter(robot_coords[0, :], robot_coords[1, :], c=["green" if matched else "red" for matched in robot_coords_were_matched], label="Robot Coordinates")
    nonzero_errors = error_scales >= MAX_ALLOWED_ERROR
    plt.scatter(robot_coords_reprojected[0, nonzero_errors], robot_coords_reprojected[1, nonzero_errors], c="blue", label="Reprojected Coordinates", alpha=0.5)
    plt.quiver(robot_coords_matched[0, nonzero_errors], robot_coords_matched[1, nonzero_errors], errors[0, nonzero_errors], errors[1, nonzero_errors], color="blue", angles='xy', scale=1.0, scale_units="xy", alpha=0.5)
    ...
    plt.scatter(pixel_coords_detected[1, :], pixel_coords_detected[0, :], c=cmap(0.3 - (quality * 0.3)), alpha=1.0, marker="x", s=50)
